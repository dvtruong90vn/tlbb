...
--事件交互入口
--**********************************
function x892105_OnDefaultEvent( sceneId, selfId,targetId )
		local	nam	= LuaFnGetName( sceneId, selfId )
		if LuaFnGetAvailableItemCount(sceneId, selfId, 10420088) >= 100 then
		BeginEvent( sceneId )
			AddText( sceneId, "   #cFF0000拥有2000个#B[喇叭]#cFF0000的玩家可在此处领取个性荣耀称号!" )	
			AddNumText( sceneId, x892105_g_scriptId, "领取个性荣耀称号", 5, 501 )
		    EndEvent(sceneId)
		   DispatchEventList(sceneId,selfId,targetId)
	      else
		   BeginEvent(sceneId)
			AddText(sceneId,"  #cFF0000注意：#W#cFF0000拥有10个#B[重楼戒],10个#B[重楼符],10个#B[重楼链]#cFF0000的玩家才可在此处换取最新四代神器!" )		
			AddNumText( sceneId, x892105_g_scriptId, "离开……", -1, x892105_g_leave)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
--事件列表选中一项
--**********************************
function x892105_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText() == 501 then
             	BeginEvent( sceneId )
		AddText( sceneId, "  #Y伟大的英雄(女俠)#W，请选择您喜欢的个性称号，让你看起来与众不同。" )
		AddNumText( sceneId, x892105_g_ScriptId, "称号 - 江湖小生·初出茅庐",7,1500 )
		AddNumText( sceneId, x892105_g_ScriptId, "称号 - 初入江湖·崭露头角",7,1501 )
		AddNumText( sceneId, x892105_g_ScriptId, "称号 - 青年才俊·锋芒毕露",7,1502 )
		AddNumText( sceneId, x892105_g_ScriptId, "称号 - 后起之秀·惊世奇才",7,1503 )
	...
	elseif GetNumText() == 1500 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,500)
	...
	elseif GetNumText() == 1501 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,800)
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 143, 0)
		DispatchAllTitle( sceneId, selfId )
		BeginEvent( sceneId )
	...
	elseif GetNumText() == 1502 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,1100)
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 143, 0)
		DispatchAllTitle( sceneId, selfId )
		BeginEvent( sceneId )
	...
	elseif GetNumText() == 1503 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,1400)
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 143, 0)
		DispatchAllTitle( sceneId, selfId )
		BeginEvent( sceneId )
	...
	elseif GetNumText() == 1504 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,1700)
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 143, 0)
		DispatchAllTitle( sceneId, selfId )
		BeginEvent( sceneId )
	...
	elseif GetNumText() == 1505 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,1900)
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 143, 0)
		DispatchAllTitle( sceneId, selfId )
		BeginEvent( sceneId )
	...
	elseif GetNumText() == 1506 then
		...
		LuaFnDelAvailableItem(sceneId,selfId,30505107,2000)
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 143, 0)
		DispatchAllTitle( sceneId, selfId )
		BeginEvent( sceneId )
