...
--**********************************
function x990001_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
	...
	AddNumText( sceneId, x990001_g_scriptId, "#gffff00#Y一键#W打孔", 2, 100)
	AddNumText( sceneId, x990001_g_scriptId, "#gffff00#Y一键#W强化", 2, 200)
	EndEvent(sceneId)
...
function x990001_OnDefaultEvent( sceneId, selfId,targetId )
	x990001_UpdateEventList( sceneId, selfId, targetId )
end
...
function x990001_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText() == 200 then
		...
		for i=bagbegin, bagend do
	       ...
	       x990001_NotifyTip( sceneId, selfId, "包裹100级以下所有装备已成功强化 1 次。" )
	...
	elseif GetNumText() == 100 then
		...
		--if find == 0 then
			--x990001_NotifyTip( sceneId, selfId, "该装备不能打孔。" )
			--return
		--end
		--if nLevel < 1 then
			--x990001_NotifyTip( sceneId, selfId, "只提供1以上的装备打孔。" )
			--return
		--end
		--if  GetItemCount( sceneId, selfId, 20500008 ) < 1 or GetItemCount( sceneId, selfId, 20501008 ) < 1 or GetItemCount( sceneId, selfId, 20502008 ) < 1 then
			--x990001_NotifyTip( sceneId, selfId, "请确保身上有足够的材料。" )
			--return
		--end
		...
		x990001_NotifyTip( sceneId, selfId, "包裹所有装备已成功打孔 1 次。" )
		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 18, 0)
	end
end
...
--接受此NPC的任务
--**********************************
function x990001_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
...
--拒绝此NPC的任务
--**********************************
function x990001_OnMissionRefuse( sceneId, selfId, targetId, missionScriptId )
	--拒绝之后，要返回NPC的事件列表
end
...
--继续（已经接了任务）
--**********************************
function x990001_OnMissionContinue( sceneId, selfId, targetId, missionScriptId )
...
--死亡事件
--**********************************
function x990001_OnDie( sceneId, selfId, killerId )
end
