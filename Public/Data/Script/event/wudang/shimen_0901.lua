...
x223901_g_ContinueInfo="Các hÕ phäi tiªp tøc c¯ g¡ng nhé!"	--Î´Íê³ÉÈÎÎñtoÕ ğµ npc¶Ô»°
x223901_g_MissionComplete="Cám ½n. "	--Íê³ÉÈÎÎñnpcËµ»°toÕ ğµ »°
...
x223901_g_Param_huan		=0	--0ºÅ: ÒÑ¾­Íê³ÉtoÕ ğµ »·Êı,TÕi ½ÓÊÕÈÎÎñÊ±ºò¸³Öµ
x223901_g_Param_ok			=1	--1ºÅ: µ±Ç°ÈÎÎñĞúng·ñÍê³É(0Î´Íê³É£»1Íê³É)
x223901_g_Param_sceneid		=2	--2ºÅ: µ±Ç°¸±±¾ÈÎÎñtoÕ ğµ ³¡¾°ºÅ
x223901_g_Param_teamid		=3	--3ºÅ: ½Ó¸±±¾ÈÎÎñÊ±ºòtoÕ ğµ ¶ÓÎéºÅ
x223901_g_Param_killcount	=4	--4ºÅ: É±ËÀÈÎÎñ¹ÖtoÕ ğµ ÊıÁ¿
x223901_g_Param_time		=5	--5ºÅ: Íê³É¸±±¾ËùÓÃÊ±¼ä(µ¥Î»:  giây)
--6ºÅ: Î´ÓÃ
--7ºÅ: Î´ÓÃ

x223901_g_CopySceneType=FUBEN_LINGXINGFENG	--¸±±¾ÀàĞÍ,¶¨ÒåTÕi ScriptGlobal.luaÀïÃæ
x223901_g_LimitMembers=1			--¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
x223901_g_TickTime=5				--»Øµ÷½Å±¾toÕ ğµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x223901_g_LimitTotalHoldTime=360	--¸±±¾¿ÉÒÔ´æ»îtoÕ ğµ Ê±¼ä(µ¥Î»: ´ÎÊı),Èç¹û´ËÊ±¼äµ½ÁË,ÔòÈÎÎñ½«»áth¤t bÕi
x223901_g_LimitTimeSuccess=500		--¸±±¾Ê±¼äÏŞÖÆ(µ¥Î»: ´ÎÊı),Èç¹û´ËÊ±¼äµ½ÁË,ÈÎÎñÍê³É
x223901_g_CloseTick=6				--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±(µ¥Î»: ´ÎÊı)
x223901_g_NoUserTime=300			--¸±±¾ÖĞÃ»ÓĞÈËºó¿ÉÒÔ¼ÌĞø±£´ætoÕ ğµ Ê±¼ä(µ¥Î»:  giây)
x223901_g_DeadTrans=0				--ËÀÍö×ªÒÆÄ£Ê½,0: ËÀÍöºó»¹¿ÉÒÔ¼ÌĞøTÕi ¸±±¾,1: ËÀÍöºó±»Ç¿ÖÆÒÆ³ö¸±±¾
x223901_g_Fuben_X=42				--½øÈë¸±±¾toÕ ğµ Î»ÖÃX
x223901_g_Fuben_Z=46				--½øÈë¸±±¾toÕ ğµ Î»ÖÃZ
x223901_g_Back_X=59				--Ô´³¡¾°Î»ÖÃX
x223901_g_Back_Z=71					--Ô´³¡¾°Î»ÖÃZ
x223901_g_TotalNeedKill=10			--C¥n É±ËÀ¹ÖÎïÊıÁ¿
...
function x223901_OnDefaultEvent( sceneId, selfId, targetId )
  ...
  elseif x223901_CheckAccept(sceneId,selfId) > 0 then		--Ã»ÓĞÈÎÎñ,Thöa mãnÈÎÎñ½ÓÊÕÌõ¼ş
...
function x223901_OnAccept( sceneId, selfId, targetId )
	if( IsHaveMission(sceneId,selfId,x223901_g_MissionId) > 0)  then	--ÒÑ¾­ÓĞÈÎÎñtoÕ ğµ 
		misIndex = GetMissionIndexByID(sceneId,selfId,x223901_g_MissionId)
...
function x223901_MakeCopyScene( sceneId, selfId, nearmembercount )
	...
	LuaFnSetSceneLoad_Map(sceneId, "lingxingfeng.nav"); --µØÍ¼Ğúng±ØĞëÑ¡È¡toÕ ğµ ,¶øÇÒ±ØĞëTÕi Config/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x223901_g_CopySceneType);--ÉèÖÃ¸±±¾Êı¾İ,ÕâÀï½«0ºÅË÷ÒıtoÕ ğµ Êı¾İÉèÖÃÎª999,ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êı×Ö×Ô¶¨Òå)
...
function x223901_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	if killednumber<x223901_g_TotalNeedKill then
		...
		for i=0,num-1 do
			...
			misIndex = GetMissionIndexByID(sceneId,humanObjId,x223901_g_MissionId) --È¡ ği¬mÈÎÎñÊı¾İË÷ÒıÖµ
			local killedcount = GetMissionParam( sceneId, humanObjId, misIndex, x223901_g_Param_killcount) --È¡ ği¬mÒÑ¾­É±ÁËtoÕ ğµ ¹ÖÎïÊı
...
function x223901_OnHumanDie( sceneId, selfId, killerId )
	if x223901_g_DeadTrans==1 then --ËÀÍöºóC¥n ±»Ç¿ÖÆÌß³ö³¡¾°
		...
		misIndex = GetMissionIndexByID(sceneId,selfId,x223901_g_MissionId)--È¡ ği¬mÈÎÎñÊı¾İË÷ÒıÖµ
...
function x223901_OnCopySceneTimer( sceneId, nowTime )
	...
	elseif TickCount == x223901_g_LimitTimeSuccess then
		...
		for	i=0,membercount-1 do
			...
			misIndex = GetMissionIndexByID(sceneId,mems[i],x223901_g_MissionId)--È¡ ği¬mÈÎÎñÊı¾İË÷ÒıÖµ
			--½«ÈÎÎñtoÕ ğµ µÚ1ºÅÊı¾İÉèÖÃÎª1,±íÊ¾Íê³ÉtoÕ ğµ ÈÎÎñ
	...
	elseif TickCount == x223901_g_LimitTotalHoldTime then --¸±±¾×ÜÊ±¼äÏŞÖÆµ½ÁË
		...
		for	i=0,membercount-1 do
			mems[i] = LuaFnGetCopyScene_HumanObjId(sceneId,i)
			DelMission( sceneId, mems[i], x223901_g_MissionId );--ÈÎÎñth¤t bÕi,É¾³ıÖ®
			if LuaFnIsObjValid(sceneId, mems[i]) == 1 and LuaFnIsCanDoScriptLogic(sceneId, mems[i]) == 1 then
  			BeginEvent(sceneId)
