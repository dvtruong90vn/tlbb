...
function x808008_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x808008_g_Key["ChgU"] then
		BeginEvent( sceneId )
			AddText( sceneId, "#B您需要去元宝店购买1个 #G更名牌 #B我就可以帮你更换名字！！！" )
...
function x808008_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x808008_g_ScriptId, "我要改名字[需要更名牌一个]", 6, x808008_g_Key["ChgU"] )
...
function x808008_IsCanChangeUserName( sceneId, selfId )
  ...
  if LuaFnGetAvailableItemCount(sceneId, selfId, 40004475) >= 1 then
    ...
    x808008_MsgBox( sceneId, selfId, "您包裹里没有更名牌,无法完成更名！请去元宝店购买." )	
	return 0
