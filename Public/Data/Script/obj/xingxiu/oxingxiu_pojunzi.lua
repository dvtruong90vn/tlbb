...
function  x016008_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"Ta có th¬ truy«n dÕy cho ğ® tØ Tinh Túc khä nång ği«u khi¬n Mao Ngßu.")
...
function  x016008_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 40 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x016008_g_ScriptId,"Có",-1,0)
      AddNumText(sceneId, x016008_g_ScriptId,"Không",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if GetNumText() == 60 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x016008_g_ScriptId,"Có",-1,1)
      AddNumText(sceneId, x016008_g_ScriptId,"Không",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
