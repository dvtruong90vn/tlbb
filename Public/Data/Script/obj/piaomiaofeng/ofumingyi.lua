...
function x402273_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		if 1 == CallScriptFunction( x402273_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "ShuangZi" ) then
			AddNumText( sceneId, x402273_g_ScriptId, "Khi陁 chi猲 B B靚h 姓o Nh鈔 c鵱g Tr醕 B Ph鄊", 10, 1 )
		end
...
function x402273_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		...
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Chi猲 黏u 5 gi鈟 sau b 馥u." )
		return
	end

	if 6 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Chi猲 黏u 4 gi鈟 sau b 馥u." )
		return
	end

	if 5 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Chi猲 黏u 3 gi鈟 sau b 馥u." )
		return
	end
	...
	if 4 == step then
		...
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Chi猲 黏u 2 gi鈟 sau b 馥u." )
		return
	end

	if 3 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Chi猲 黏u 1 gi鈟 sau b 馥u." )
		return
	end

	if 2 == step then
		--提示Chi猲 黏u b 馥u.....
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Chi猲 黏u b 馥u." )
		return
	end
