...
x002010_g_ActivateMissionId = 8			-- ¼¤»îÌõ¼þ
x002010_g_SongXinScriptId = 006668		-- ËÍÐÅ
...
function x002010_UpdateEventList( sceneId, selfId, targetId )
  local PlayerName = GetName( sceneId, selfId )
	...
	BeginEvent( sceneId )
		AddText( sceneId, "  " .. PlayerName .. PlayerSex .. ", th§t hâm mµ ngß½i hành t¦u giang h° thiên mã vÕn trß¶ng. Ta ðây chï · trong thành, th§t phi«n lòng thay..." )
...
function x002010_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- Nhi®m vø Mµc Uy¬n Thanh
		if IsHaveMission( sceneId, selfId, x002010_g_RSMissionId ) > 0 then
			x002010_NotifyFailBox( sceneId, selfId, targetId, "Ngß½i vçn chßa hoàn thành nhi®m vø trß¾c ðó." )
			return
		end
		...
		if nDayTime == CurDaytime  then -- µ±Ìì
			if nCount >= 100  then
				BeginEvent( sceneId )
					AddText( sceneId, "Hôm nay ta ðã làm phi«n ngß½i nhi«u. Ngß½i v« ði, ngày mai hãy quay lÕi." )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
				return
			end
